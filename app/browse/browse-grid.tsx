...
export function BrowseGrid() {
  ...
  }, [serviceType, searchTerm, fuelType, brand, seats, priceRange, sortOrder, allCars, allDrivers]);
          ...
          <div className="p-4 bg-card rounded-lg border space-y-6 sticky top-24">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input placeholder={serviceType === 'Cars' ? "e.g., Toyota RAV4" : "e.g., John Doe"} className="pl-10" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
              </div>
            </div>
                ...
                <AnimatedSection stagger className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    {Array.from({ length: 6 }).map((_, i) => (
                        <motion.div variants={fadeInUp} key={`${serviceType}-${i}`}>
                            {serviceType === 'Cars' ? <CarCard car={null} /> : <DriverCard driver={null} />}
                        </motion.div>
                    ))}
                </AnimatedSection>
                ...
                filteredCars.length > 0 ? (
                    <AnimatedSection stagger className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                        {filteredCars.map((car) => (
                            <motion.div variants={fadeInUp} key={car.id}><CarCard car={car} /></motion.div>
                        ))}
                    </AnimatedSection>
                ) : (
                ...
                filteredDrivers.length > 0 ? (
                    <AnimatedSection stagger className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                        {filteredDrivers.map((driver) => (
                            <motion.div variants={fadeInUp} key={driver.id}><DriverCard driver={driver} /></motion.div>
                        ))}
                    </AnimatedSection>
                ) : (
                    <AnimatedSection className="flex flex-col items-center justify-center h-full text-center py-16 border-2 border-dashed rounded-lg">
